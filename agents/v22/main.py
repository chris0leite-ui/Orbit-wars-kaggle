...
# v22: import lite_greedy_policy under a different name; the real
# `_opp_policy` symbol below wraps it with counter-recapture moves.
# Keeping the lib symbol untouched preserves v7_X / v15 / v20 behavior
# (per `explicit-rewrite-of-implicit-behavior` friction: do NOT in-place
# edit a shared lib function; substitute at the call site instead).
...
# ---------------------------------------------------------------------------
# v22 — counter-recapture-aware opp policy
# ---------------------------------------------------------------------------

# Radius (game-units) within which a defender planet is "near" the
# threatened planet for counter-recapture purposes. Matches the
# `NEIGHBORHOOD_RADIUS` from v21's E2 hold-check (the same physics
# pattern — local force dominates the recapture battle).
_COUNTER_RADIUS = 30.0


def _opp_policy(obs):
    """v22 opp policy = lite_greedy_policy + counter-recapture overlay.

    The base (`_lite_greedy_offensive`) is unchanged from `lib.opp_model`.
    On top we append one defensive counter-launch per threatened opp
    planet, when an incoming my-fleet is targeting it AND a nearby opp
    planet has surplus garrison to neutralise the threat.

    Counter-recapture rule (one per threatened planet per turn):
      For each MY in-flight fleet `f`:
        1. Find the OPP planet `P` nearest to f's current (x, y), within
           `_COUNTER_RADIUS`. If none, skip.
        2. Find the OPP planet `S != P` nearest to `P` within
           `_COUNTER_RADIUS`, with `garrison(S) − 1 ≥ ships(f)`, that has
           not already emitted this turn. If none, skip.
        3. Emit `[S, atan2(P − S), ceil(ships(f)) + 1]`.

    `obs` is the opp's seat observation; `'me' from this policy's POV`
    is `obs.player`. The function is symmetric — it gets called per
    seat in `_opp_actions_for_snap` below.

    Wallclock: O(my_fleets × opp_planets) per call; on a typical
    mid-game board (~10 fleets × 15 planets = 150 checks at ~0.5 µs
    each) = ~75 µs per call. With ~30 rollout steps × per-step opp
    calls, adds ~2 ms per candidate. Well inside the budget.

    Ported from v21's `_opp_actions_for_snap_with_counter` (which lived
    inside the now-falsified E2 hold-check). Verified to fire correctly
    by the v21 instrumentation gate. The only behavioural change vs v21
    is *where* it's called: previously only in the once-per-candidate
    hold-check; now in EVERY rollout step, so the leaf F2 naturally
    collapses on fragile captures the opp would actually counter.
    """
    base = list(_lite_greedy_offensive(obs) or [])
    try:
        seat = int(obs.player if hasattr(obs, "player")
                   else obs.get("player", 0))
        raw_planets = (obs.planets if hasattr(obs, "planets")
                       else obs.get("planets", []) or [])
        raw_fleets = (obs.fleets if hasattr(obs, "fleets")
                      else obs.get("fleets", []) or [])
        # Own planets (this seat is the policy's "me" for one call).
        own_planets = [p for p in raw_planets if int(p[1]) == seat]
        if not own_planets:
            return base
        own_by_id = {int(p[0]): p for p in own_planets}
        used_srcs = {int(m[0]) for m in base}
        R2 = _COUNTER_RADIUS * _COUNTER_RADIUS
        additions = []
        for f in raw_fleets:
            if int(f[1]) == seat:
                continue  # only counter against attackers (other seats)
            f_ships = float(f[6])
            fx = float(f[4]) if len(f) > 4 else 0.0
            fy = float(f[5]) if len(f) > 5 else 0.0
            # 1. Threatened own planet: nearest within radius.
            threatened = None
            best_d2 = R2
            ...
            if threatened is None:
                continue
            tpx, tpy = float(threatened[2]), float(threatened[3])
            tpid = int(threatened[0])
            # 2. Best defender: nearest own planet to threatened with
            #    surplus and not already used.
            best_src = None
            best_src_d2 = float("inf")
            for sp in own_planets:
                spid = int(sp[0])
                if spid == tpid or spid in used_srcs:
                    continue
            ...
            ships_to_send = max(2, int(math.ceil(f_ships)) + 1)
            additions.append(
                [int(best_src[0]), float(angle), int(ships_to_send)]
            )
            used_srcs.add(int(best_src[0]))
        return base + additions
    except Exception:
        # Any error → fall back to bare offensive. Rollout never raises.
        return base
