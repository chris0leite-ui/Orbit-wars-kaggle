...
def _agent_inner(obs, configuration=None):
    ...
    gambit = False            # set in the offense block; read by the veto
    stalemate = False
    if time.perf_counter() - t0 < TIME_BUDGET:
        ...
        if gambit:
            pressure = 0.0
        # true stalemate: projection-behind in a frozen game (no fleets in
        # the air on either side) — quiet play rides a fixed gap to a
        ...
        def plan_attack(tgt):
            ...
            for k in range(1, len(members) + 1):
                ...
                if stalemate:
                    # a certain loss is the baseline — admit ANY
