...
from lib.opp_model import lite_greedy_policy as _opp_policy_lite
from lib.opp_model import top_tier_mirror_policy as _opp_policy_top
...
# v13: hybrid opp policy in _build_opp_trajectory. First N steps use
# top_tier_mirror_policy (v3.5.1 pipeline — accurate model for the
# strategically critical opening window where compounding losses
# originate); steps thereafter use lite_greedy_policy (~10x cheaper,
# accuracy matters less once the rollout is far enough out that the
# leaf state's prod_share is the dominant signal).
#
# 2P cost: 10*10ms + 20*1ms = ~120ms per turn (vs ~30ms with pure
# lite_greedy). v12 bench p95=116ms; v13 expected p95 ~200-300ms — well
# under 1000ms actTimeout. 4P: ~360ms (3 opps); may need to reduce
# OPP_TRAJ_TOP_TIER_STEPS if bench shows over-budget.
OPP_TRAJ_TOP_TIER_STEPS = 10
...
def _build_opp_trajectory(snap_base, me, num_seats, max_horizon):
    ...
    v13 hybrid policy: top_tier_mirror_policy (v3.5.1 aggressive snipe
    + reinforce, ~10ms/call) for steps 0..OPP_TRAJ_TOP_TIER_STEPS-1 —
    the strategically critical opening window where compounding
    expansion decides the game. lite_greedy_policy (~1ms/call) for
    subsequent steps, where accuracy matters less because the leaf
    state's prod_share / ship_balance dominate the Δ signal.
    ...
    Cost (2P): top_tier 10*10 + lite_greedy 20*1 = ~120ms per turn at
    max_horizon=30. (4P: ~360ms with all 3 opps; watch wallclock.)
    """
    ...
    for step_i in range(max_horizon):
        if snap.fake_env.done:
            ...
            continue
        policy = (
            _opp_policy_top if step_i < OPP_TRAJ_TOP_TIER_STEPS
            else _opp_policy_lite
        )
