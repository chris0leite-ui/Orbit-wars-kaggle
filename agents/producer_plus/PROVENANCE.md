...
## Vendor pin (claude/blissful-cray-tiusnw, 2026-06-12)

This copy of `agents/producer_plus/` + `agents/producer/orbit_lite/` was
vendored from `claude/awesome-clarke-ixy57v` at commit `be71c97` — the
commit the live garval submission (sub 53588922) was built from. Pinned
to the live-submission state on purpose: the A/B control for the learned
shot filter must byte-match what is on the ladder. New in this tree on
top of be71c97: `shot_mlp.py` (learned shot-success filter + trained
weights), the `PRODUCER_PLUS_SHOT_MLP` gate in `main.py`, and the
namespaced-prelude support in `scripts/bundle_producer_plus.py`.
