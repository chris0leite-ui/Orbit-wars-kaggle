"""v4.6_drop_smallest — v4_planner + drop_smallest_launch portfolio.

v4_planner with one surgical change: `include_drop_smallest=True` adds a
6th portfolio that drops the single mission with the fewest ships
(across all sources). This is Lesson 1 from the v4.5_robust postmortem
— a fine-grained "hold the most marginal launch back" perturbation
that differs from v4's existing `drop_weakest_source` (which drops an
entire source's worth of missions). One extra Sim call per turn (≤80ms);
p95 turn stays under v4's 753ms budget.

Per-turn pipeline (unchanged from v4_planner except step 3):
...
3. **Portfolios**: `generate_portfolios(..., include_drop_smallest=True)`
   enumerates up to 6 mission-list variants (incumbent / conservative /
   per_source_swap / drop_weakest_source / **drop_smallest_launch** /
   noop). Each is run through `settle_plan` and `realize` to produce
   a candidate action.
...
def agent(obs, configuration=None):
    ...
    portfolios = generate_portfolios(
        world, model, incumbent_missions, include_drop_smallest=True,
    )
