...
# v14: maximin over physics-grounded archetypes.
# Top-K candidates from the validate stage are re-scored against 5
# archetype trajectories (no_launch / v351 / counter_reinforce /
# counter_snipe / cross_attack), each consisting of the archetype's
# step-0 spoiler action followed by lite_greedy for steps 1..K-1.
# Maximin aggregation picks the candidate that does best against its
# worst-case archetype response. Opponent strategy is NOT assumed —
# only opp's physical resources are.
_K_MAXIMIN_2P = 3
_K_MAXIMIN_4P = 2
...
def _build_archetype_traj(snap_base, me, num_seats, max_horizon,
                          archetype_fn, our_intents=None):
    """Pre-compute opp trajectory where step 0 is the archetype's spoiler
    response and steps 1..max_horizon-1 are lite_greedy continuations.

    The archetype is a physics-grounded "what could opp do to hurt this?"
    response — it does NOT assume an opp policy, only opp's resources.
    Beyond step 0 we don't keep extrapolating worst-case (combinatorial)
    — lite_greedy is the cheap, generic continuation. Variance reduction
    via CRN: same trajectory replayed for baseline + every candidate.
    """
    snap = fs_clone(snap_base)
    out = []
    for step_i in range(max_horizon):
        ...
        for opp_id in range(num_seats):
            if opp_id == me:
                continue
            try:
                obs_raw = snap.state[opp_id].observation
                if step_i == 0:
                    obs_pov = opp_pov_obs(obs_raw, opp_id)
                    if archetype_fn is archetype_counter_reinforce:
                        actions[opp_id] = archetype_fn(obs_pov, our_intents) or []
                    else:
                        actions[opp_id] = archetype_fn(obs_pov) or []
                else:
                    actions[opp_id] = _opp_policy(obs_raw) or []
            except Exception:
                actions[opp_id] = []
        out.append(actions)
        snap = fs_step(snap, actions, in_place=True)
...
def agent(obs, configuration=None):
    ...
    candidates = []  # validated (delta, src, tgt, ships, angle, wait_N, horizon)
    for _cheap, src, tgt, ships, angle, _eta, horizon, wait_N in top:
        ...
        if delta > 0:
            candidates.append((delta, src, tgt, ships, angle, wait_N, horizon))

    if not candidates:
        return []

    # ---------------------------------------------------------------
    # Stage 4: maximin over physics-grounded opp archetypes.
    # For the top-K validated candidates, score against 5 archetype
    # trajectories. Pick the one with the highest WORST-CASE score
    # across all archetypes. Opp strategy is NOT assumed — opp's
    # action set is bounded by their actual ships/planets/ETA via
    # the archetype definitions in lib/missions/opp_archetypes.py.
    # ---------------------------------------------------------------
    ...
    if len(short_list) >= 2 and time.perf_counter() < t_hard_cap:
        ...
        our_intents = [
            ...
            for c in short_list if int(c[5]) == 0  # only fire-now intents
        ]
        ...
        # Per-archetype scoring loop with soft budget guard. If we
        # blow the deadline, fall back to the top-of-validated pick
        # (= v12 behavior).
        ...
        for fn in archetype_fns:
            ...
            try:
                arch_traj = _build_archetype_traj(
                    snap_base, me, num_seats, MAX_HORIZON, fn, our_intents,
                )
                arch_baseline = _build_idle_baseline(
                    snap_base, me, num_seats, MAX_HORIZON, arch_traj,
                )
            except Exception:
                continue
            for i, (_delta, src, _tgt, ships, angle, wait_N, horizon) in enumerate(short_list):
                ...
                try:
                    d = _score_action(
                        snap_base, me, num_seats,
                        int(src.id), angle, ships,
                        horizon, arch_baseline, arch_traj, wait_N=wait_N,
                    )
                    scores[i].append(d)
                except Exception:
                    scores[i].append(-1e9)

        # Maximin pick: candidate with the largest minimum score.
        worst = [min(s) if s else -1e9 for s in scores]
        best_i = max(range(len(short_list)), key=lambda i: worst[i])
        # Reorder candidates so the maximin pick is at position 0 for
        # the greedy emit below.
    ...
    # Note: maximin reordering above has already placed the worst-case-
    # robust pick at position 0; we PRESERVE that order (don't re-sort).
    used_srcs, used_tgts = set(), set()
    moves = []
    for _delta, src, tgt, ships, angle, wait_N, _horizon in candidates:
        sid = int(src.id)
        tid = int(tgt.id)
