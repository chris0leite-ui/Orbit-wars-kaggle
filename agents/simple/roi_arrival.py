"""roi_arrival — ROI scored on PROJECTED arrival garrison, not current.

V1 in the strategy A/B test. Replaces the snapshot `target.ships` with
the deterministic projection `target.ships + production * eta` for
enemy/neutral targets via `lib.scoring`. The hypothesis is that scoring
on projected garrison shifts targeting away from "high-yield but
will-be-fortified-by-arrival" planets.

Score: production / (projected_garrison + distance + 1).
