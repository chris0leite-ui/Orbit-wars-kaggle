...
# ---------------------------------------------------------------------------
# Favor (F1 + F2 + F4 vulnerability) — v15 leaf scorer
# ---------------------------------------------------------------------------

# F4 vulnerability penalty (v15 Iteration 1):
# A planet whose production we score "owned" is functionally lost if an
# enemy fleet lands before we can defend. The default _favor counts
# such planets at face value -- a systematic over-valuation that becomes
# the leaf-scorer saturation surface (two v9-family agents see the same
# face-value landscape and pick the same moves).
#
# F4 fix: weight each planet's production contribution by how secure
# it is. For each planet owned by `owner`, compute the earliest ETA at
# which an opposing-color fleet (in-flight or potential-launch from a
# stationary enemy planet) could land. If threat_eta < F4_THREAT_HORIZON
# discount production linearly toward 0 as threat_eta approaches
# F4_THREAT_DEADLINE. Naturally symmetric: opp planets we threaten get
# the same discount, so we correctly value future captures too.
#
# Computed from raw obs (planets + fleets tuples) to avoid per-leaf
# WorldModel rebuild. O(planets * (planets + fleets)) per favor() call;
# in 2P ~10 * (10 + 10) = 200 ops, ~1us. Negligible.
F4_THREAT_HORIZON = 8
F4_THREAT_DEADLINE = 3
F4_DECAY_WIDTH = 5.0


def _planet_threat_eta(planet, planets, fleets, owner):
    """Earliest ETA an OPPOSING fleet could land on `planet`.

    Considers (a) in-flight enemy fleets aimed roughly at the planet,
    (b) potential launches from each stationary enemy planet's current
    garrison. Cheap straight-line distance / speed proxy; the rollout
    simulator catches actual orbital misses.
    Returns a large int (10**9) if no plausible threat.
    """
