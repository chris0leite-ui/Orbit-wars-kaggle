...
## 2026-06-03
- `tag: vh-magnitude-swamp-survives-clean-model` — Phase D retrained the value head from scratch on 100 state-K self-play games. Training gates passed clean (Spearman ρ=+0.386 on val, σ(label)=497.8, walker parity exact). The wrapper at λ=1.0 still collapsed 0/32 vs the rolling state-K champion — same as every prior λ=1.0 attempt on the broken Jun-2 model. **Root cause:** σ(pred)≈100 swamps base score O(10-100) regardless of model quality. **Fix:** model quality alone cannot rescue an additive integration with mismatched scales; if rerank (Option C) also fails, the entire VH-on-state-K wiring is closed (it did, see next entry).
- `tag: vh-rerank-also-fails-despite-rank-signal` — Phase H consumed the validated rank signal (ρ=+0.386) as pure rank: top-K=10 solos re-sorted by head_out, base score preserved, emit gate unchanged. n=32 vs rolling champion = 2/32 = 6.2% (Wlo=0.017, Whi=0.201). vs random = 16/16 = 100%, so no wiring bug. **Root cause hypotheses:** K=10 ship-delta target is a noisier local approximation of what leaf-delta + PV-discount already integrate over 10-30 turns; corpus rows are pre-filtered by the same chooser they're meant to rerank, minimal marginal info; src/tgt collision-aware greedy emit is sensitive to top-K ordering. **Fix:** close the VH-on-state-driven-K integration axis (6 falsifications: 4 additive on broken model + 1 additive fresh + 1 rerank fresh); next-session pivot per state/MULTI_BRANCH.md Closed tracks.
- `tag: corpus-gen-not-on-this-branch` — Phase D blocked at start because `scripts/gen_b2_corpus.py` was referenced in HANDOVER and prior plans but doesn't exist on `claude/champion-ml-graft-majestic-storm`. The grafted branch dropped the entire B.1/B.2 trace infrastructure (gen_b2_corpus.py, probe_pveta_selfplay.py, _trace_hook.py, chooser trace_accepted calls). **Root cause:** branch graft cherry-picked the inference code paths (`_value_head.py`, `value_head_features.py`) but not the corpus-gen scaffold that builds the training data. **Fix:** ported all four pieces from `claude/competition-objective-alignment-hqNVM` commit 9d32066 (~600 LOC); committed at `1a8818f`. Future-proofing rule: when grafting a learned-model code path across branches, **also graft the data pipeline that produced the model**, or the model becomes unreproducible.

## 2026-06-04
- `tag: mechanism-family-cross-stack-transfer-needs-planner-compat` — Three falsifications of pressure-gradient ship circulation (v1 centroid scalar 5/16 + wallclock 2958 ms; v2 Biel-style distance-decayed enemy mass 8/16 + max 1424 ms; v3 v2 + destination-usefulness filter regressed back to 5/16 + max 1396 ms). **Root cause:** the lever is a thin post-pass; Biel's "Producer" agent uses an identical mechanism successfully because his entire planner thinks in pressure (same scalar field feeds his attack scoring AND his regroup destinations). Our chooser thinks in (source, target) trade scoring with no pressure notion. Pressure-routed ships land at destinations our chooser ignores. **Fix:** future ship-utilization work requires either a chooser-internal rewrite (pressure-aware scoring) or a goal-directed 2-hop pre-positioning approach (chooser-aware by construction); do not propose another thin gradient-based post-pass without showing how the chooser would consume the pre-positioned ships. See `audit/2026-06-04-postmortem-champion-ml-graft-majestic-storm.md` and `knowledge-base/flags/2026-06-04-ship-utilization-still-open.md`.
- `tag: tighter-filter-regresses-not-improves` — frontier_circulation v3 added a strictly tighter destination filter (only fire when destination has a capturable enemy in its 8 nearest) over v2 — intuitively should improve things, since blocked launches were the "obviously wasted" ones. INSTEAD regressed 8/16 → 5/16. **Root cause:** v2's "wasted" launches were apparently providing incidental value (defensive density at the front, pre-positioning for future captures the chooser eventually finds) that v3's narrower logic doesn't capture. Recurrence of the chooser-family-saturation pattern: when local tightening regresses on a co-tuned baseline, the substrate is wrong and further tuning won't recover. **Fix:** when v_n→v_{n+1} adds a strictly tighter filter and REGRESSES, stop tuning the lever family — the substrate is the problem, not the filter. Same lesson as `chooser-family-saturation` 2026-05-16.

- `tag: empty-actions-faster-than-vanilla-is-silent-crash` — producer_plus Step 4 first revision had `clamp(min=float, max=Tensor)` which is invalid PyTorch syntax; plan_lite_waves threw, agent returned `_empty_entries`, n=32 came in at 1/64. The only clue was wallclock: p50=14ms vs vanilla 22ms (FASTER than vanilla). Cost: ~45 min A/B compute on what was a crash, not a regression. **Fix:** when adding code to a planner's hot path, a per-turn p50 wallclock LOWER than the vanilla baseline on the same hardware is a red flag for swallowed exceptions; smoke a full game and inspect action rows before any A/B.
- `tag: migration-step-validate-vs-target-not-just-prior-step` — producer_plus Step 4 (multi-size) beat the Step 2+4 baseline 13/16 (81%) but lost to vanilla producer 5/16 (31%). Transitivity (A>B, B≈C ⇒ A>C) doesn't hold in adversarial games. **Fix:** every migration step must include a direct A/B vs the migration TARGET (vendor's vanilla agent), not just vs the prior step on the same engine.
- `tag: cross-wave-over-drain-from-shared-budget` — producer_plus Step 4 broke a SILENT invariant of producer's greedy: `cand_send == drain` for every candidate meant at-most-one wave per source, so over-drain was impossible. Multi-size variants smaller than drain let multiple waves fire from one source totaling more than safe_drain. **Fix:** when adding a new candidate-set axis (multi-size, multi-source, wait_N), audit the greedy selector's per-source budget invariants for accidents of single-variant uniformity; cap source_budget at drain explicitly, don't rely on per-candidate uniformity.
