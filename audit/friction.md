...
## 2026-06-03
- `tag: grep-pipe-buffers-ab-monitoring` — piped a long A/B (`bash run_ab.sh 2>&1 | grep -vE noise`) and watched a status monitor read the pipe target; `grep` block-buffers, so the file stayed near-empty for ~14 min and the run looked stalled when it was healthy (8 games in flight). **Fix:** never pipe a live long-running job through `grep` for monitoring — `exec >raw.log 2>&1` and filter on read; `python -u` for the inner job. `clean_ab` already streams per-game `wall=Xs` lines, which is the right progress signal.
- `tag: parity-baseline-missing-config-header` — built the "before-my-edits" parity bundle with no champion-config header, so it ran `main.py`'s internal defaults while the test arm carried the 13-key champion header → false divergence at step 1 (a CONFIG mismatch, not a code diff). Burned one ~8-min parity run. **Fix:** a parity baseline must carry the IDENTICAL config header as the arm under test and differ only by the code/flag being verified; build both from the same base via a clean worktree (`git worktree add --detach HEAD`).
- `tag: ab-timeout-too-short-for-full-games` — set `timeout 540` on a bundle self-play test whose 3 full 500-step games need ~21 min; it got SIGTERM'd (exit 143) twice and looked like a test failure. **Fix:** size timeouts to the real per-game cost (~7–8 min/full game); when "no-crash full game" is already proven elsewhere (94 completed A/B games here), run only the fast integrity assertions (`-k "... and not self_play"`) instead of re-paying for the slow self-play.
- `tag: pkill-sleep-sandbox-blocked-exit144` — `pkill -f` and foreground `sleep` both return exit 144 in this sandbox, killing the whole bash block. **Fix:** stop processes with `python -c "os.kill(pid, SIGTERM)"` after enumerating via `ps`; never foreground-`sleep` — use `Monitor`/`run_in_background` poll loops or task notifications to wait.
