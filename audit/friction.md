...
## 2026-05-12 (simplify-codebase-p39Hm)

- `tag: state-files-out-of-date-vs-live-mu` — session start: state/current.md reported v3.5.1 PENDING with predicted μ=1090-1100 and σ-equivariance v1 at μ=976.3. Actual live μ pulled via `kaggle competitions submissions orbit-wars` was v3.5.1=994.7 and σ-equiv=1063.2 — both off by 60-100μ in opposite directions. PI had to push me to check live scores; I would have planned the v7 merge against stale priors otherwise. **Fix:** at session-start, before reading state/, run a one-shot `curl -s -u $KAGGLE_USERNAME:$KAGGLE_KEY https://www.kaggle.com/api/v1/competitions/submissions/list/orbit-wars` and reconcile against state-file claims.
- `tag: local-A/B-not-live-mu-predictor` — v3.5.1 aggressive sizing measured 68.8% Wilson lo 56.6% at 32-seed 2P vs frozen v3_snipe (audit/2026-05-12-iter2-ablation-results.md). Live ladder result: μ=994.7, **below** v3_snipe baseline at 1005.7. σ-equivariance was framed as "μ-floor only" but landed at μ=1063.2, **above** every other submission. Local→live correlation has been poor twice this comp. **Fix:** stop quoting expected-μ from local-A/B winrates in HANDOVER/state files. Quote only the local winrate + Wilson CI; let PI judge ladder transfer separately.
- `tag: misdiagnosed-v7-self-play-break` — observed 8/16 = 50% draws in v7 self-play (vs σ-equiv-only's 16/16). Hypothesised env-RNG drift between processes and applied a `cfg["seed"] = obs.step` "fix" to env_from_obs. Post-fix self-play: 8W/0L/8D each direction — bit-identical to pre-fix. The env was already deterministic via the tournament harness's `make(configuration={"seed":...})` call; my inner-env seed was a no-op. Real root cause: maximin in a payoff matrix without a pure saddle point produces opposing picks for P0 and P1 (max-min vs min-max diverge on mixed-strategy turns). **Fix:** before applying a "fix" to an empirical-symmetry break, write a 10-line probe that compares the actual artifact (here: payoff matrices computed by P0 vs P1 on a single divergent turn) to verify the hypothesis. The diff would have shown the matrices were identical, ruling out RNG drift in <5 min.
- `tag: thin-local-gates-drove-500-LOC-merge` — game-theory branch reported v7 vs v3.4 = 6W/0D/2L and v7 vs precision_v3 = 6W/0D/2L from 8 games each. PI authorised "merge v7 in full" based on those. Our 32-seed retest of v7 vs frozen v3_snipe was 17/32 = 53.1% Wilson [36.4%, 69.1%] — statistical tie with 50% and **worse** than the σ-equiv-only HEAD's 18/32 = 56.2%. 8-game gates with Wilson [37%, 95%] are too noisy to authorise +500-LOC integration. **Fix:** gate any "+>200 LOC merge" decision behind ≥32 games of A/B vs the strongest extant baseline. Repeat 8-game gates on the local branch should not promote to a different branch's integration decision.
- `tag: accepted-upstream-correctness-claim-at-face` — game-theory commit `cbed49e` claimed "σ-equivariance preserved via the existing lib/planner patches. Two v7 instances in self-play should produce identical payoff matrices → mirror picks → draws." This is false when the payoff matrix has no pure saddle point: max-min ≠ min-max in mixed cases. I quoted the claim into HANDOVER without checking the math. **Fix:** when an upstream commit asserts a structural property (preserves X, monotone in Y, …), re-derive it from first principles before relying on it in a different context.
