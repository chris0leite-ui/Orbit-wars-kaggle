...
## 2026-06-03 (claude/kaggle-submission-strategy-JzIAr — teamwork-seam reversal + refine submit)
- `tag: ab-on-non-champion-base` — the first refine winrate A/B was run with adaptive-K OFF (fixed horizon 10) on both sides — a non-champion config. It produced a misleading 68.8%-vs-62.5%-noisy-parity read; I nearly concluded "no lift." PI caught it ("did you consider the adaptive-K version?"). Re-basing on the live champion config gave a clean 78%-vs-50.0% parity. **Fix:** A/B on the LIVE champion config (`state/MULTI_BRANCH.md` rolling pair), not the repo default; confirm the parity anchor is ~50% BEFORE trusting any lift number.
- `tag: env-default-misread-as-off` — claimed "I'm not using the kinematic table" because my launcher didn't set `BASELINE_KINEMATIC_TABLE` explicitly; in fact it defaults `"1"` (ON) at `main.py:896`, so both sides had it the whole time. Gave the PI a wrong answer for one turn. **Fix:** "I didn't set the flag" ≠ "the flag is off" — grep the `os.environ.get(...)` default before asserting a config state.
- `tag: tmp-artifacts-lost-on-restart` — saved A/B replays (`/tmp/refine_adaptivek_replays`, `/tmp/refine_panel_replays`) and the running Rule-43 panel were all wiped by a container restart; the regression-tail diagnosis corpus is gone and the panel weak-legs never finished. **Fix:** for artifacts needed next session, save under a path that survives (committed or repo-tracked dir), not `/tmp`; for long gates, expect restarts and checkpoint or run in resumable chunks.
- `tag: env-run-stderr-sandboxed` — `kaggle_environments.env.run` swallows each agent's stdout/stderr into its own per-step log, so an outer `redirect_stderr` captured nothing; two smoke runs read as "refine generated 0 coalitions" when really the debug never reached me. **Fix:** to measure an agent's INTERNAL signal, call the function directly on reconstructed boards (`World.from_obs`), don't rely on agent prints surfacing through `env.run`.

## 2026-06-04 (claude/kaggle-submission-strategy-JzIAr — opening-wait diagnostic)
- `tag: leaned-toward-hypothesis-before-testing` — opened by partly endorsing the horizon-K-starvation read (and even drafted a "shipped fix is under-powered" line from the K_OPEN=20 < median-neutral-ETA-22 docstring) before measuring. The direct test refuted it outright (0/31 opening turns starved; 0% maps past K_OPEN=20). The docstring stat was about ALL boards, not the opening-reachability question. **Fix:** when a plausible mechanism story is offered, build the discriminating measurement FIRST and let it speak; don't pre-commit to the framing or quote a stat that answers an adjacent question.
- `tag: both-seats-propose-aggregate-too-slow` — the full-game × both-seats × per-board `propose()` aggregate blew the 300s timeout on a single seed (each board's `propose` is heavy; two full games + 60 propose calls > cap). **Fix:** split the cheap map-level question (step-0 reach, no game, no `propose`) from the expensive behavioral question (launch overlay, one focal game); never run `propose()` per board across a multi-seat panel under one timeout.
- `tag: argv-parse-before-mode-check` — added a `--scan` mode but the `SEEDS = [int(s) ...]` parse ran first and crashed on the placeholder `x` arg. **Fix:** parse mode flags / use tolerant `_safe_int` BEFORE positional int-coercion when a script has multiple invocation modes.
