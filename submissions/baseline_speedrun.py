# Bundled by scripts/bundle_agent.py from agents/baseline + lib/* (see header below).
# Prepended env stack: BASELINE_VALUE_HEAD=speedrun + orbitfix knobs + speedrun-head calibration.
# Source commit: favor_speedrun (planet-count-first leaf for 4P FFA elimination push).
# PI directive 2026-05-26 PM: "Go build the speedrunner. Aim for elimination in most games."
from __future__ import annotations
import os as _bs_os
_bs_os.environ.setdefault("BASELINE_JOINT_AGGR", "1")
_bs_os.environ.setdefault("BASELINE_JOINT_TOP_K", "5")
_bs_os.environ.setdefault("BASELINE_JOINT_MAX_PAIRS", "60")
_bs_os.environ.setdefault("BASELINE_REINFORCE_EMIT", "1")
_bs_os.environ.setdefault("BASELINE_REINFORCE_ANTICIPATE", "1")
_bs_os.environ.setdefault("BASELINE_NEUTRAL_BONUS", "2.0")
_bs_os.environ.setdefault("BASELINE_NEUTRAL_EARLY_EXTRA", "1.5")
_bs_os.environ.setdefault("BASELINE_NEUTRAL_EARLY_HORIZON", "50")
_bs_os.environ.setdefault("BASELINE_ORBITAL_SAFETY", "1")
_bs_os.environ.setdefault("BASELINE_VALUE_HEAD", "speedrun")
_bs_os.environ.setdefault("BASELINE_SPEEDRUN_K_PLANET", "50")
_bs_os.environ.setdefault("BASELINE_SPEEDRUN_K_ACQUIRE", "0.1")
_bs_os.environ.setdefault("BASELINE_SPEEDRUN_K_SHIPS", "0.5")
_bs_os.environ.setdefault("BASELINE_SPEEDRUN_HOLD_HORIZON", "20")
_bs_os.environ.setdefault("BASELINE_SPEEDRUN_REACH_HORIZON", "25")
...
def enumerate_wave_candidates(my_planets, target_pool, world, model,
                              me: int, omega: float) -> list:
    """Multi-source coordinated wave candidates (baseline_wave v3 2026-05-24).

    Returns `list[(target, [(src, ships, angle, wait_N), ...])]`. Each wave
    is a coordinated attack on a single target where multiple sources fire
    at different times so all fleets land on the same step — exploiting
    combat rule 1 (additive stacking).

    Default OFF via `BASELINE_WAVE_PROPOSER`; returns `[]` when off so the
    orbitfix-peak path is byte-identical. The existing JOINT-from-prerank
    enumeration at `chooser_trajectory.py:961-1012` stays as a fallback for
    non-wave-producing turns.
    ...
    for tgt in target_pool:
        ...
        if int(tgt.owner) == me:
            continue
        sources = nearest_k(my_planets, tgt, K)
        # Probe eta_S per source at a capture-size budget; sources with
        # < MIN_FLEET_SIZE ships skipped (can't contribute).
        per_src: list[tuple] = []
        for S in sources:
            if int(S.ships) < MIN_FLEET_SIZE:
                continue
            probe_ships = max(MIN_FLEET_SIZE,
                              capture_size(S, tgt, model, omega, me, world))
            if probe_ships > int(S.ships):
                # Source can't afford the probe fire-now; size to its budget
                # for the eta estimate (still a useful rough eta).
                probe_ships = int(S.ships)
            _angle0, eta_s = aim_and_eta(
                S, tgt, probe_ships, omega, wait_N=0, world=world,
            )
            per_src.append((S, int(eta_s), int(probe_ships)))
        if len(per_src) < 2:
            continue
...
def select_favor_fn():
    ...
    # from lib.value_heads import stockpile_pressure_penalty  # inlined by bundle_agent.py
...
def agent(obs, configuration=None):
    ...
    if os.environ.get("BASELINE_CHOOSER", "").strip().lower() == "trajectory":
        ...
        # from agents.baseline.proposer import enumerate_wave_candidates  # inlined by bundle_agent.py
