...
# === agent ===
"""v22_sun_on_v15 — v15 chooser (live μ=1115.5) + proposer-time sun pre-filter.

Smallest possible defensive addition to the live-best agent. v15's
chooser, value function, wait-N grid, banded dedup, K-rollout — all
unchanged. The single change: candidates whose straight-line trajectory
crosses the sun safety zone or terminates out of bounds are dropped
BEFORE entering the prerank pool.

Rationale (2026-05-17): live ladder data shows v15=1115.5, v20=1095.5,
v21≈v20 (46.9% h2h). v20 LOST 20μ vs v15 live despite local 65.6%,
suggesting v20's dogpile-emit has unseen-opponent failure modes. v22
builds on the proven live-winner instead of the local-winner.

By construction (defensive-only addition), v22 can only ever DROP
candidates that v15 would have validated. Worst case: rollout would
have correctly scored them as negative-Δ (so they wouldn't emit
anyway), and v22 ≡ v15 bit-for-bit. Best case: v15's rollout was
emitting some sun-bound candidates (Rule 38 evidence: across 8
seeds, v20 fires ~0.35% of fleets into the sun; v15 has the same
chooser pipeline architecture, so similar rate expected), and v22
spares those wasted ships.

Pipeline: v15-original; only the prerank append guard is new.
    ...
    for src in my_planets:
        ...
        for tgt in _nearest_k(target_pool, src, NUM_TARGETS_PER_SOURCE):
            ...
            for ships in _enumerate_ship_counts_basic(src, tgt, model, omega, me, world):
                ...
                # v22: drop sun-bound / OOB candidates BEFORE prerank.
            ...
            for w_ships, w_wait_N, w_angle, w_eta in _wait_then_fire_candidate(
                ...
                # v22: drop sun-bound / OOB wait-N candidates too.
