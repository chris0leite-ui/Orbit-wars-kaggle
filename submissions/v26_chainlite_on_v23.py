...
# === agent ===
"""v26_chainlite_on_v23 — v23 (sun fate) + chain bonus @ 1/3 weight.

Rule 37 closure ablation #3 (proposer-axis). Identical to v25 EXCEPT
the chain_bonus weight is monkey-patched from CHAIN_BONUS_WEIGHT=0.30
down to 0.10 (1/3 size).

Hypothesis test: is the regression magnitude-driven or mechanism-driven?
- v24 (rotation @ 0.02 weight): -10pp vs v23 baseline
- v25 (chain @ 0.30 weight): pending
- v26 (chain @ 0.10 weight): tests "smaller magnitude, same mechanism"

If v26 also regresses, conclusion is mechanism-driven: ANY proposer-
stage bonus addition shifts prerank ordering enough to harm validate-
budget selection regardless of magnitude. That fully falsifies the
proposer-bonus axis under Rule 37 (3 of 3 ablations failed).

If v26 is neutral/positive while v25 regressed, conclusion is
magnitude-driven: smaller bonuses might survive. That re-opens the
axis for further tuning at lower weights.

Original v25 docstring follows:
...
# v26: 1/3 of stock 0.30 chain weight. Patched via the function's
# module __globals__ so it survives both dev imports AND the bundler
# (which inlines lib.compound's globals into the bundle's top-level
# namespace; this reassignment then overrides the inlined value).
_compound_bonus.__globals__["CHAIN_BONUS_WEIGHT"] = 0.10
