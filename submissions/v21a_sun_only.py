...
# === agent ===
"""v21a_sun_only — v20 dogpile + proposer-time sun-safe pre-filter ONLY.

Single-axis ablation isolating v21_compound's defensive component. No
compound_bonus, no MissionBook persistence. The only change from v20:

In the prerank loop, candidates whose straight-line trajectory crosses
the sun (within SUN_RADIUS + 0.5 cushion) or terminates out of bounds
are dropped BEFORE entering the validate stage.

Rule 38 evidence (from v21_compound build session, 2026-05-16): v20
sends ~0.35% of fleets into the sun across 8 seeds (6 sun + 2 oob /
1700 launches); v21's pre-filter brings that to zero. v21a isolates
whether this defensive lift alone is enough to clear v20 on h2h.

Pure defensive change — by construction, it can only DROP candidates
that v20 would have validated. Worst case: the rollout would have
correctly Δ-scored those sun-bound candidates as negative and dropped
them anyway, in which case v21a is bit-equivalent to v20. Best case:
the rollout was scoring some of them as positive (and emitting them),
and removing them frees the chooser to consider better alternatives.
"""
...
# from lib.compound import fleet_path_safe  # inlined by bundle_agent.py
    ...
    for src in my_planets:
        ...
        for tgt in _nearest_k(target_pool, src, NUM_TARGETS_PER_SOURCE):
            ...
            for ships in _enumerate_ship_counts_basic(src, tgt, model, omega, me, world):
                ...
                # v21a: drop sun-bound / OOB candidates BEFORE prerank append.
            ...
            for w_ships, w_wait_N, w_angle, w_eta in _wait_then_fire_candidate(
                ...
                # v21a: drop sun-bound / OOB wait-N candidates too.
