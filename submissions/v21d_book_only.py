...
# === agent ===
"""v21d_book_only — v20 dogpile + MissionBook TTL persistence ONLY.

Single-axis ablation: v20 prerank, then add a small TTL-decayed bonus
to candidates whose (src, tgt) matches a live commit. Commits get
recorded after dogpile emit. No sun pre-filter, no rotation, no chain.
"""
...
# from lib.compound import compound_bonus  # inlined by bundle_agent.py
...
def agent(obs, configuration=None):
        ...
        for tgt in _nearest_k(target_pool, src, NUM_TARGETS_PER_SOURCE):
            ...
            for ships in _enumerate_ship_counts_basic(src, tgt, model, omega, me, world):
                ...
                cheap += compound_bonus(
                    ...
                    anchor_xy=None, mission_book=_mission_book, wait_N=0,
                    use_rotation=False, use_chain=False, use_carry=True,
                )
                if cheap > -10.0:
                    prerank.append(
            ...
            for w_ships, w_wait_N, w_angle, w_eta in _wait_then_fire_candidate(
                ...
                w_cheap += compound_bonus(
                    src, tgt, w_ships, w_eta, world, model, me,
                    anchor_xy=None, mission_book=_mission_book, wait_N=w_wait_N,
                    use_rotation=False, use_chain=False, use_carry=True,
                )
                if w_cheap > -10.0:
                    prerank.append(
    ...
    # v21d: register chosen pairs for next-turn carryforward bonus.
