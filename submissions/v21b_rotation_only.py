...
# === agent ===
"""v21b_rotation_only — v20 dogpile + rotation_alignment bonus ONLY.

Single-axis ablation: v20 prerank, then add 0.02 * production * alignment
as a bonus where alignment is in [-1, 1] measuring how much the target
drifts toward my-cluster centroid over 30 turns. No sun pre-filter, no
chain bonus, no MissionBook.
"""
        ...
        for tgt in _nearest_k(target_pool, src, NUM_TARGETS_PER_SOURCE):
            ...
            for ships in _enumerate_ship_counts_basic(src, tgt, model, omega, me, world):
                ...
                cheap += compound_bonus(
                    ...
                    anchor_xy=anchor_xy, mission_book=None, wait_N=0,
            ...
            for w_ships, w_wait_N, w_angle, w_eta in _wait_then_fire_candidate(
                ...
                w_cheap += compound_bonus(
                    src, tgt, w_ships, w_eta, world, model, me,
                    anchor_xy=anchor_xy, mission_book=None, wait_N=w_wait_N,
                    use_rotation=True, use_chain=False, use_carry=False,
                )
                if w_cheap > -10.0:
                    prerank.append(
