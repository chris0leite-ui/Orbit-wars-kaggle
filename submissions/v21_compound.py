...
# === agent ===
"""v21_compound — v20 dogpile chooser + proposer-time compound augmentations.

What changed from v20 (identical chooser, identical leaf scorer):

1. Sun-safe pre-filter in the prerank loop. Candidates whose straight-
   line path from src to (orbital-lead) arrival point would cross the
   sun (with 0.5-unit cushion) are dropped BEFORE entering prerank.
   v20 relied on realize-time mechanism.sun_avoid; that left the rollout
   wasting budget on doomed candidates and let through unsafe paths
   when the prerank cap selected them. Pre-filter is cheap
   (`lib.geometry.path_clears_sun` ≈ 1 µs per call).

2. Compound bonus added to `_cheap_marginal_value`. Three components,
   all model-derived (Rule 40):
   - rotation_alignment × production: planets drifting toward our
     cluster centroid get a bonus (longer expected hold).
   - chain_bonus: if capturing this target unlocks an affordable
     follow-on capture within 15 turns, add a fraction of the
     follow-on's PV.
   - carryforward bonus: if (src, tgt) matches a live commit in
     mission_book, small stability nudge.

3. MissionBook persistence: after the dogpile emit chooses moves, each
   (src, tgt) pair is committed with a TTL of 3 turns. Carried-forward
   commits get the carryforward bonus on next turn's scoring pass,
   reducing per-turn churn. Commits expire when (a) TTL hits 0, (b)
   src is lost, or (c) target's ownership flips to us.

Untouched (per 5/16 postmortem — value-function asymmetric-compounding
axis exhausted): `_favor`, `_score_action`, the K-rollout loop, the
reactive opp model, the dogpile emit logic. The lift, if any, comes
from BETTER CANDIDATES reaching the validate stage — not from
re-weighting them at the leaf.
"""
...
# from lib.compound import compound_bonus, fleet_path_safe  # inlined by bundle_agent.py
...
fleet_speed = speed
# from lib.geo.rotation import my_cluster_centroid  # inlined by bundle_agent.py
# from lib.intent import World  # inlined by bundle_agent.py
# from lib.mission_book import BOOK as _mission_book  # inlined by bundle_agent.py
_mission_book = BOOK
...
def agent(obs, configuration=None):
    ...
    # v21: maintain MissionBook across turns. New game (step==0 OR step
    # decreased vs last call) wipes state. Carryforward bonus for live
    # commits is applied inside the prerank loop via compound_bonus.
    _mission_book.reset_if_new_game(int(world.step))
    _ = _mission_book.carryforward(world, model, me)
    anchor_xy = my_cluster_centroid(my_planets)
    ...
    for src in my_planets:
        ...
        for tgt in _nearest_k(target_pool, src, NUM_TARGETS_PER_SOURCE):
            ...
            for ships in _enumerate_ship_counts_basic(src, tgt, model, omega, me, world):
                ...
                # v21: drop sun-bound / OOB candidates BEFORE prerank append.
                if not fleet_path_safe(src, angle, ships, eta):
                    continue
                ...
                # v21: compound-aware augmentation (rotation + chain + carry).
                cheap += compound_bonus(
                    src, tgt, ships, eta, world, model, me,
                    anchor_xy, _mission_book, wait_N=0,
                )
                if cheap > -10.0:
                    prerank.append(
            ...
            for w_ships, w_wait_N, w_angle, w_eta in _wait_then_fire_candidate(
                ...
                # v21: drop sun-bound / OOB wait-N candidates too. The path
                # is computed at FIRE time (after rotation by omega*wait_N),
                # but we drop based on the eventual fleet path geometry which
                # is what fleet_path_safe checks via _aim_and_eta's outputs.
                if not fleet_path_safe(src, w_angle, w_ships, w_eta):
                    continue
                ...
                w_cheap += compound_bonus(
                    src, tgt, w_ships, w_eta, world, model, me,
                    anchor_xy, _mission_book, wait_N=w_wait_N,
                )
                if w_cheap > -10.0:
                    prerank.append(
    ...
    # v20: dogpile-permitted emit. Max 1 launch per source per turn
    # (avoids draining a source on two parallel actions); MULTIPLE
    # sources MAY target the same planet (dogpile allowed). v15's
    # per-target dedup capped emit at 1-3 per turn even when 6-16
    # positive-Δ candidates were available, because top-Δ candidates
    # cluster on a few high-value targets. Each candidate's Δ was
    # already validated independently by the rollout, so the 2nd
    # launch at the same target has been judged net-positive given
    # the 1st landing — dogpile is self-balancing under the validated-Δ
    # filter. Diagnostic: Forrest replay step 80/150/190 had 9/6/6
    # positive-Δ candidates → 3/2/2 emits (v15) due to target dedup;
    # without target dedup, expect ~5-9 emits at those steps, closing
    # the gap to opp's observed 5-11 emit-rate.
    candidates.sort(key=lambda c: -c[0])
    used_srcs = set()
    moves = []
    chosen_commits = []  # (src_id, tgt_id, score, tgt_owner)
    for delta, src, tgt, ships, angle, wait_N in candidates:
        sid = int(src.id)
        if sid in used_srcs:
            continue
        used_srcs.add(sid)
        chosen_commits.append((sid, int(tgt.id), float(delta), int(tgt.owner)))
        if wait_N == 0:
            moves.append([sid, float(angle), int(ships)])
        # else: wait-N picked → reserve src/tgt, emit nothing this turn.

    # v21: register chosen (src, tgt) pairs in the mission book so they
    # get the carryforward bonus on next turn's scoring pass. Decay
    # outstanding TTLs AFTER commit so this turn's emits start with full
    # TTL. Wait-N picks are committed too — the agent has decided to
    # reserve that src for this target, and we want next turn's chooser
    # to keep that reservation rather than flip to a new candidate.
    for sid, tid, delta, tgt_owner in chosen_commits:
        _mission_book.commit(sid, tid, delta, int(world.step), tgt_owner)
    _mission_book.decay_ttls()
    return moves
