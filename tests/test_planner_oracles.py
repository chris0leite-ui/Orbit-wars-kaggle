...
@pytest.mark.xfail(
    strict=False,
    reason=(
        "Known upstream gap: the current chooser does not emit the "
        "coordinated A-captures + B-reinforces sequence; it sees A's "
        "low residue and abstains. This oracle was 1/14 failing on the "
        "source branch (claude/strategy-framework-design-OyoYR-rebased) "
        "at cherry-pick time (2026-05-29). Keeping the test as an "
        "explicit hole rather than a silent skip — flipping to PASS is "
        "the gate for declaring the multi-source coordination axis closed."
    ),
)
