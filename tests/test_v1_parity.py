...
@pytest.fixture(scope="module")
def parity_agent():
    """Build an agent callable pinned to the parity-preserving mechanism set.

    `DEFAULT_MECHANISMS` grows as 3.5.B/C/D land (arrival_size, comet_aim,
    sun_avoid) and the live v1.x agent's behaviour shifts. The parity test
    must NOT shift — it asserts the strategy + foundational mechanisms
    (validate + lead_aim) still reproduce pre-refactor v1 exactly. So we
    construct a callable here that ignores DEFAULT_MECHANISMS and uses
    only the pinned PARITY_MECHANISMS subset.
    """
    from agents.v1_orbitfix.main import propose_intents
    from lib.intent import realize
    from lib.mechanism import PARITY_MECHANISMS

    def agent(obs):
        return realize(propose_intents(obs), obs, mechanisms=PARITY_MECHANISMS)
...
def test_v1_rewards_match_pre_refactor_vs_baseline(pre_refactor_v1, parity_agent, seed):
