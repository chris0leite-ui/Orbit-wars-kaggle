"""Parity gate for the ledger agent's future forecast.

The ledger agent's premise is that, absent new launches, it can predict
every planet's (owner, ships) exactly, arbitrarily far ahead — orbit
rotation, comet paths and expiry, fleet flight with swept-disk collision,
production order, and combat resolution all included.

This test runs a real engine episode where both players launch scripted
fleets early and go silent, then checks the agent's World.build_ledger()
forecast from several mid-game observations against the engine's actual
later states. Any mismatch in owner or ship count is a failure.

Forecast windows are chosen to stop short of comet spawn steps (50, 150,
250...) because future comets are unpredictable from the observation (the
episode seed is not visible to agents).
"""
...
def _load_ledger():
    spec = importlib.util.spec_from_file_location(
        "ledger_agent_under_test", REPO / "agents" / "ledger" / "main.py")
