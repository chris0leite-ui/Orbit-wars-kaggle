...
# ---------------------------------------------------------------------------
# Phase 6: chain-capture bonus on the Claws relay opening (step 223)
# ---------------------------------------------------------------------------


def test_claws_step223_chain_bonus_promotes_p3_p31(claws_step223, monkeypatch):
    """POSITIVE REGRESSION — Phase 6 chain-bonus on the Claws relay.

    Step 223 of ep 77164175 is the pre-launch state Claws sees before
    emitting `[3, 1.005, 588]` (the p3 → p31 leg of the relay chain).
    p3 has ~784 ships, p31 (enemy, prod=1) has 43, and p23 (enemy,
    prod=1) is one short hop from p31. Without the bonus, capturing the
    prod=1 p31 looks low-value; with the bonus, the chain to p23 (and
    other reachable enemy planets) shows up in cheap_delta and the
    candidate is tagged is_chain=True.
    """
    from kaggle_environments.envs.orbit_wars.orbit_wars import Planet, Fleet
    from lib.intent import World
    from lib.world_model import WorldModel
