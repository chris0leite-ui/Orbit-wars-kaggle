"""Tests for Step 3 (redux) of producer_plus: opponent-projection scorer.

Covers Rule 38 (fix-verification reproduces failure state):
    1. ``_opp_projection_enabled`` env gate parses the flag correctly.
    2. With the gate UNSET, action rows are byte-identical to vanilla
       ...
       failure mode).
    3. Smoke: max per-turn wallclock under 1000 ms (Rule 46).
"""
...
def producer_plus_main():
    ...
    spec = importlib.util.spec_from_file_location(
        "producer_plus_main_test_opp_proj",
        os.path.join(PRODUCER_PLUS_DIR, "main.py"),
    )
    module = importlib.util.module_from_spec(spec)
    sys.modules["producer_plus_main_test_opp_proj"] = module
    spec.loader.exec_module(module)
    return module


def test_opp_proj_default_off(monkeypatch, producer_plus_main):
    monkeypatch.delenv("PRODUCER_PLUS_OPP_PROJECTION", raising=False)
    assert producer_plus_main._opp_projection_enabled() is False


@pytest.mark.parametrize("value", ["1", "true", "True", "yes", "on", "ON"])
def test_opp_proj_env_on_truthy(monkeypatch, producer_plus_main, value):
    monkeypatch.setenv("PRODUCER_PLUS_OPP_PROJECTION", value)
    assert producer_plus_main._opp_projection_enabled() is True


@pytest.mark.parametrize("value", ["0", "false", "no", "off", ""])
def test_opp_proj_env_off_falsy(monkeypatch, producer_plus_main, value):
    monkeypatch.setenv("PRODUCER_PLUS_OPP_PROJECTION", value)
    assert producer_plus_main._opp_projection_enabled() is False
...
def test_off_path_bit_identical_to_producer():
    """Rule 38 reproduce-failure test: with PRODUCER_PLUS_OPP_PROJECTION
    unset (default OFF), producer_plus must emit the same action rows as
    vanilla producer at fixed seeds. This guards against ``the opp-projection
    branch accidentally perturbed the static-opp scorer fallback`` regressions.
    """
    os.environ.pop("PRODUCER_PLUS_OPP_PROJECTION", None)
...
@pytest.mark.slow
def test_opp_proj_on_smoke_wallclock_under_1s():
    """Rule 46 smoke: run one full game with opp-projection on, assert
    total wallclock under 60 s as a loose per-turn proxy.
    """
    shim_path = os.path.join(PRODUCER_PLUS_DIR, "producer_plus_opp_proj.py")
    producer_path = os.path.join(PRODUCER_DIR, "producer_agent.py")
    t0 = time.time()
    state, steps = _play_one_game(shim_path, producer_path, seed=7)
    elapsed = time.time() - t0
    assert elapsed < 60.0, (
        f"opp_projection game vs producer at seed 7 took {elapsed:.1f}s — "
