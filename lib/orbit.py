...
def predict_relative_cached(current_planet, angular_velocity: float,
    ...
    Phase β of /root/.claude/plans/do-it-thoroughly-consider-tingly-fox.md.
    ...
    or predicted planet states (e.g. inside `aim_orbiting`'s
    fixed-point loop where the "planet" position is a hypothetical
    future tick) MUST pass `table=None` to force the slow path; the
    table's API surfaces this via a missing-pid check + IndexError
    on out-of-range lookups, but callers are responsible for not
    handing the table synthetic data.
    """
    if table is None:
        return predict_relative(current_planet, angular_velocity, lead_turns)
    # Probe pid in a way that works for both Planet dataclass and
    # raw env-tuple [id, owner, x, y, radius, ships, production].
    pid_obj = None
    try:
