...
|---|---|---|---:|---|---|
| 2026-05-23 19:50 | extract-physics-trajectory-Vjaz9 | **orbitfix_kt_p23 v5 (perf: inline rotation in predict_fleet_fate)** — same content as ERROR'd sub 52963659 (attack-pull positional value head + comet-aware fleet_target_planet), plus a perf fix: inline the rotation math in `lib/trajectory.predict_fleet_fate`'s per-planet position pre-compute loop (commit 3c39fd1). Avoids 53M predict_relative calls per 4P game (one math.hypot + math.atan2 per planet instead of per-(planet,lead) pair). Local 4P seed=0 profile: p50 dropped 1266ms → **921ms** (27%), turns >1s dropped 72% → **32%**, game went from "lost in 189 turns" to "**WON in 499 turns**" (full game completed). Root cause of sub 52963659 ERROR: ~60s overage budget exhausted by turn 203 from 132 turns over actTimeout. With p50 now under 1s, overage usage projects to ~19s of the 60s budget — plenty of headroom. | ~1000-1080 (likely improves vs sub 52959167 (μ=984.4) due to (a) game completion vs ERROR, (b) attack-pull lift if real, (c) the comet-aware fix is correctness hygiene) | 52949672 (μ=984.3) | ✅ PI explicit "submit then 3" |
| 2026-05-23 17:40 | extract-physics-trajectory-Vjaz9 | **orbitfix_kt_p23 v4 (attack-pull positional value)** — same env stack as sub 52959167, plus `BASELINE_VALUE_HEAD=attack_pull` activating a NEW positional leaf term. `_attack_pull_ship_value(obs, me)` weights each of my ships (on-planet + in-flight) by closeness to the nearest ENEMY planet — forward-deployment incentive addressing PI's "agent is too passive" diagnosis. Cheap variant (`favor` base + attack-pull term; NOT the composite-based `favor_hybrid_attack_pull` which blew p95→1931ms). Plus comet-aware fleet_target_planet from cmt 289d8ed (development-hygiene fix). Local n=16 A/B vs sub 52959167 bundle: 12/16 (75%) Wilson [0.505, 0.898] PASS. Wallclock p50=264ms p95=543ms (max=1867ms — one spike, investigating). **OUTCOME: ERROR** (60s overage exhausted in 4P validation; p50 hit 1.15s vs the 1s actTimeout). | ~1050-1100 (75% A/B WR vs sub 52959167 μ=963 implies ~190 Elo / ~100 μ gap → predicted ~1060+) | 52949672 (μ=973.6) | ✅ PI explicit "submit then 3" |
| 2026-05-23 15:25 | extract-physics-trajectory-Vjaz9 | **orbitfix_kt_p23 v3 (predict_relative + comet fix)** — same env stack as sub 52949672, plus two physics-correctness fixes: (1) `lib/orbit.predict_relative` honors `is_orbiting` for static planets (commit 1ad6cfa); previously always rotated, producing wrong positions for static targets in `lib/aim.py` and `agents/baseline/main.py` reinforce paths. (2) Comet-aware position dispatch via new `lib.world_model.planet_position_at` in baseline reinforce emit paths (commit 4c80932). KT bit-parity test dropped 81% → 40% turn divergence after fix (1); fix (2) closes reinforce-path comet sites. Remaining ~40% lives in chooser / opp_model / score_rollout (in-rollout scoring), tracked for next iteration. Bench p95=790ms, max=1012ms; bundle 10/10 GREEN; smoke 209-turn game WIN vs v7_0. | ~1000-1150 (no fresh A/B; predicted ≥ previous μ=981.8 because both changes are correctness fixes that improve aim accuracy for static + comet targets) | 52937273 (coord v3, μ=954.7) | ✅ PI explicit "submit first" |
| 2026-05-23 09:20 | extract-physics-trajectory-Vjaz9 | **orbitfix_kt_p23 v2** — same env stack as sub 52935965 + 4 commits of code-review fixes: Phase 4 Δ-calibration (6e20a23), Phase 3b skip_planet_id (1fc2073), per-call env reads + cost-model corrections + defensive guards + cheap_delta criticality + hoisted hot-loop imports (8f29020), pre-existing test repair (8a7f0c9). Previous sub on broken code settled μ=1036.8 (few games); this resubmits on corrected code. | ~1050-1150 (no fresh A/B; predicted ≥ previous μ=1036.8 because all changes are correctness fixes on a previously-broken implementation) | 52937273 (coord v3, μ=955.7) | ✅ PI explicit "go" |
| 2026-05-22 21:55 | extract-physics-trajectory-Vjaz9 | **orbitfix_kt_p23** — orbitfix substrate + kinematic table + Phase 1 (deterministic chooser, commit a50eb8e) + Phase 2 (adaptive K, fcf1a73) + Phase 3 (leaf in-flight fate check, 25589ad) | ~1100-1200 (vs orbitfix μ=1161: n=8 5W/3L=62.5% Wilson [0.31, 0.86]; vs coord μ=916.5: n=6 partial 5W/1L=83% so far) | 52927313 (coord, μ=916.5) | ✅ PI explicit "submit" (Rule 45 n=32 overridden) |
