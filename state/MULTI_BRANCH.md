...
| 2026-06-15 20:47 | claude/practical-hamilton-oth767 | `pp_positional` = seq_strength 1280 base + `TERMINAL_PROD_VALUE=12` — the **positional objective**: producer's exact static-opp 18-step forward sim, leaf values *position* (ships **+ economy/production owned at the horizon**) not raw ships ("best position vs opponent in 18 steps"). Single fresh game on producer's exact engine (seed 11): positional **beat** ship-flow 26-0 planets — cures the flow leaf's banking/under-expansion. ⚠️ flat `terminal_prod=12` was previously refuted on both referee classes ("rewards expansion the opponent punishes before payback"); deliberate re-test on the moved field. | **UNCERTAIN** — prior refutation ⇒ likely < 1194; fresh single game ⇒ objective helps. n=1, vs a non-punishing opponent. | evicts **53711823** `pp_ws` (wideshortlist) μ **1194.3 settled** (older of the rolling pair; Kaggle evicts by age, not score). New pair `{pp_positional, pp_expand 53714433 μ 960.9 warming}`. | ✅ PI **"Submit it"** (2026-06-15) — explicit override of the Rule 42 block (evicted 1194.3 > predicted/uncertain). Rule 46: producer_plus bundle builds + full game max **180 ms** < 1000, beats v7_0; the 6 `test_bundle` errors are pre-existing **lib-bundler** (`bundle_agent.py`/`kinematic_table`) infra, unrelated to producer_plus. |
| 2026-06-16 06:12 | claude/practical-hamilton-oth767 | `pp_pos4p` = seq_strength 1280 + `TERMINAL_PROD_VALUE_4P=12` — the **positional objective GATED TO 4P ONLY** (`_terminal_prod_for(player_count)`: positional in 4P, ship-flow in 2P). Grounded in the 2P/4P asymmetry: positional LOSES in 2P (n=32: 0.38 vs ship-flow champion) but HELPS in 4P (n=32: term12 **0.38 vs 0.25 baseline**; term6/18 0.19 — weight-sensitive, 12 is the sweet spot). **2P byte-identical to champion** (n_steps 106/98/114). Dominates champion (= 2P, ≥ 4P). Fixes the pp_positional mistake (positional applied globally → 2P regression → 1062). | ≈ champion (~1188–1205) in 2P + marginal 4P upside. Low risk (2P byte-safe). | evicts **53714433** `pp_expand` (~957, older of rolling pair); new pair `{pp_pos4p, pp_positional 1062.7}` — floor ~957→~1188+. | ✅ PI **"Go"** (2026-06-16). Rule 42 **GREEN** (evicted 957 < predicted ~1188). Rule 46: builds; 2P parity byte-identical; latency max **83 ms** (2P) / ~180 ms (4P) < 1000. |
