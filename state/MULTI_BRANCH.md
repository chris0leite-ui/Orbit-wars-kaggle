...
|---|---|---|---:|---|---|
| 2026-06-12 23:50 | happy-babbage-6j46p6 (RL track) | `rl_v7_selfplay_league.py` — transformer policy/value (120k params, pure-numpy inference) trained by PPO self-play + league on the parity-tested JAX engine, 95M env-steps over 3 GPU runs. sha256 `b7fa033da3c16820`, 341,273 B. Purpose: **live calibration of the RL track** (matchmaking vs the 1300+ field that no local opponent represents). Local: 30/32 vs v7_0 (Wilson-lo 0.80), 24/32 vs ledger_v1_4 (Wlo 0.58), ~25% vs live_garval rebuild at v6 stage (v7 n=16 read in flight). | ~1100-1280 (decisively beats v7_0-class; below producer_plus-class) | **53594710** `oracle_rw` μ = 1018.7 (older half, warming since 08:12 same day). Backstop stays: **53595717** `shotmlp015` μ = 1283.8. Evicted 1018.7 < predicted floor ~1100 → **Rule 42 GREEN**. | ✅ PI explicit "Submit for live ladder testing" (2026-06-12 23:31 UTC). Rule 46 adapted for single-file RL agent: numpy/JAX parity suite 6/6 green today; `fast.py play` vs v7_0 seed 7 → WIN in 171 steps, max turn 92 ms << 1000; kaggle-env self-vs-self validation DONE/DONE. |
