...
|---|---|---|---:|---|---|
| 2026-06-02 04:30 | competition-objective-alignment-hqNVM | baseline_pv_eta_vh_dist_jsr_addone (bundled, NOT SUBMITTED; commit 5bda9a2). v7_search.choose(enumerator_mode="add_one", K=10, wallclock_ms=500) as aggression-mode handoff from trajectory chooser when ship advantage crosses 1.5×. Rule 46 GREEN: test_bundle.py 10/10; resolver unit tests 12/12; seed=0 smoke max=929ms inside cap, 144/144 v7_choose invocations successful (after bundle name-collision fixes: `v7_search_choose` alias + `_v7_score_candidate` rename). Local A/B vs jsr: **11/16 = 68.8% Wilson [0.444, 0.858]** INCONCLUSIVE (max seeds). Local A/B vs champion `baseline_launch_rules_universal_local`: **0/16 = 0.0% Wilson [0.000, 0.194]** FAIL. Rule 43b submission gate NOT cleared. Rule 42 floor risk: rolling-pair entries μ=792-815 and 537.7; addone predicted μ~829-1000 (jsr-line range) would evict the stronger of the two for an unknown lift. NOT SUBMITTED per PI sign-off "wrap up + commit fixes, no submit". | ~829-1000 (jsr-line range, +22pp local vs jsr but 0/16 vs champion) | (no submit) | ✅ PI explicit "wrap up + no submit" 2026-06-02 04:30 UTC |
