...
|---|---|---|---:|---|---|
| 2026-05-31 17:35 | competition-objective-alignment-hqNVM | baseline_pv_eta_vh_dist (PENDING SUBMIT) — pv_eta + distilled-ladder Tier-2 opp model (lib/opp_model.py:trained_logreg_policy v2), 30-d lite feature encoder via lib/opp_features_lite.encode_lite_batch; booster trained on 50,482 positives from 775 top-10 Kaggle ladder 2P replays (2026-05-30 daily dataset); KINEMATIC_TABLE_ENABLED hard-disabled in lib/trajectory.py (PI directive d8dad76). Rule 46: bundle 781KB, test_bundle.py + fast.py play in budget (single-game vs v7_0 winner 192 steps p95=790ms/max=868ms; vs launch_rules winner 341 steps p95=779ms/max=843ms). Local n≥4 A/B impossible in-session (vs launch_rules ~7min/game, n=32 ~7.5h); single-game wins both vs v7_0 AND vs launch_rules (n=1 each, very noisy). | unknown — single-game wins both vs v7_0 (μ ~1100-1200) AND vs launch_rules (μ ~1100). Posterior range very wide: 950-1200. Mean estimate ~1100. | 53212044 (baseline_pv_eta_vh_b3smoke, μ=**1147.0**) — **μ-LOSS eviction predicted** | ✅ PI explicit "submit" 2026-05-31 17:35 |
