...
## Open track — value-leaf / ME-defends (2026-06-03, `champion-strategy-rules-00JzI`)
| **ME-defends (rollout self-policy) TRIAGE NEGATIVE — does not beat the champion.** PI redirected to the value-leaf axis (highest leverage: shared substrate under the champion). Target: the audited hoarding/territorial-undervaluation (audit/2026-06-01 — agent out-ships but under-captures; "fix the valuation, not a hoarding cap"). Root cause: leaf read after ~13-tick rollout with ME idle while opps react ⇒ defensible expansion looks fragile. Fix tried = **ME-defends** (`BASELINE_ME_DEFENDS=1`, already built, default-OFF, byte-identical off; future-me plays purely-defensive reactions in the CANDIDATE rollout only, asymmetric baseline). **Mechanism verified** (tests/test_chooser_me_defends.py: DEFENDS raises the expansion candidate's score by the held-territory value; 3 tests green). **Timing OK** (bench p95 900 vs champion 878, both WATCH, max 945 < 1000 — DEFENDS ≈ champion, +22ms). **A/B: DEFENDS-champion vs champion (CRN, both nokt bundles, only diff = DEFENDS) = 5/16 (31.2%), Wilson [0.142,0.556]** — clearly NOT a lift (needs Wlo≥0.50; point estimate below parity). One game also lost (seed 0). Echoes the FALSIFIED ME-reacts: making future-me "act" in the rollout distorts candidate rankings more than the hoarding fix helps. | tests/test_chooser_me_defends.py (Rule-38 repro, 3 green). submissions/baseline_champ_defends.py = nokt champion + DEFENDS hardcoded ON (A/B fork). Bench: DEFENDS p50 568/p95 900/max 945; champion p50 421/p95 878/max 945. A/B b7qd0la63 (n=16, 1218s). | **Throughput wall:** champion-vs-champion A/Bs are ~2min/game (heavy bundles); n=16 vs v7_minimax timed out (850s), n=16 champ-vs-champ timed out (1500s); only n=8/w=4 (→16 games) finished in 1218s. v7_0 saturated (champion 16/16). **Decision pending (PI):** (a) Phase-B hold-gated capture credit (value-leaf fallback, but axis has long falsification history — Rule 37 caution); (b) test DEFENDS vs a weak-expander opp (istinetz-like) where hoarding actually costs — but no local such opponent; (c) close ME-defends, pick a different axis; (d) build a faster A/B path (decided-lead early-call / lighter opponent) before any more value-leaf A/Bs. |
