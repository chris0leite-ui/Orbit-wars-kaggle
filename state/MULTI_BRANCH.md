...
|---|---|---|---|
| Marco-EAM opp model + one-ply adversarial re-rank (axis-1 of opening-exploit family) | `claude/game-theory-winning-strategy-SEU7P` | 2026-06-02 | Phase-1 parity gate PASS at 84.6% (port reproduces marco-v3-3's beam search faithfully). Rerank gate fires 60% of turns; PROMOTES top-1 only 14% of calls, all top-1-vs-top-2 marginal-V swaps. n=8 vs flags-OFF anchor: 4W/4L = 50%, Wilson [0.22, 0.78] — FAILS Rule 45 Wilson-lo ≥ 0.50. n=13 vs romantamrazov marco-fork: 5W/8L = 38.5%, Wilson [0.18, 0.64] — FAILS Rule 43c ≥ 0.55. Two real rerank bugs found and fixed mid-session (wait-turn lite_greedy substitution + horizon-scale mismatch); fixes correct in isolation but did not move the n=8 needle vs marco-v3-3 self (3W/5L = 37.5%). **Verdict:** when opp's planner is depth-5 capture-time minimisation, the EAM commits ARE the obvious-best moves; knowing them in advance doesn't yield a different best-response. Rule 37 axis-1 closed. Durable artefacts kept: `lib/opp_marco.py` (faithful port, reusable Tier-3 component), `lib/opp_model.py` Tier 3 cached policy wiring, `scripts/probe_rerank_{fires,promotes}.py` no-op-detection diagnostics, `scripts/parity_marco_port.py` parity gate, `scripts/build_marco_variants.py` env-var-gated A/B variant builder, wait-turn + horizon fixes in `chooser_trajectory.py`. See `audit/2026-06-02-marco-lineage-reference/ab_vs_*.log` for partial logs. Knowledge-base: `knowledge-base/thoughts/2026-06-02-marco-opening-exploit-axis-falsified.md`. NEXT opening-exploit attempt must be a different axis (multi-turn opp lookahead, opp-pool distribution model, deception layer) — not another tuning of this rerank. |
...
|---|---|---|---:|---|---|
| 2026-05-30 08:23 | game-theory-winning-strategy-SEU7P | baseline_redeploy_gangup (sub **53177486**) — forward-redeploy + gang-up-support proposer generators + hybrid_spatial value head; on top of PV_ETA wrapper | ~1100-1200 (n=4 = 3/4 = 75% vs frozen anchor μ=1163.5, Wilson [0.30, 0.95] — wildly underdetermined; submit IS the live-ladder calibration) | **53163774 (baseline_pv_eta, μ=1111.4 → 1085.6 drift)** — note: pre-submit Rule 42 check ran 9h before submit; sibling branch sub 53175658 (baseline_launch_rules_k10) landed in between but I did NOT re-check the rolling pair immediately before submit. Friction: same class as 2026-05-20 cross-branch coordination gap that originated Rule 42. Cost: ~11pp floor loss vs claimed eviction. | ✅ PI explicit "submit to kaggle so I can observe" |
