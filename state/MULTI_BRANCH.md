...
|---|---|---|---:|---|---|
| 2026-05-22 22:30 | consolidate-codebase-refactor-dQAWA | coord v2 — bundle-market coord + Day-13 fix stack: smooth-ΔW endgame bonus (λ_W=0.002, default ON), code-review fixes (leaf-floor=0.0, attack/defend kind gates, EPISODE_STEPS drift assert, COORD_REDUCED_FLOOR knob default 0), deadline-bounded enumerate (timing probe showed 84%→77% idle, p50 720ms→524ms under 600ms budget) | ~900-1050 (n=4 swapped vs orbitfix @ μ=1165 = 1W/3L IDENTICAL outcomes to baseline coord but faster wins; predicted broadly similar to old coord μ=905.6 ± noise — main lift is the budget fix which only helps mid-late game) | 52927313 (coord, μ=905.6 — self-eviction; new variant replaces weak old one) | ✅ PI explicit "submit the current solution now" |
| 2026-05-22 14:00 | consolidate-codebase-refactor-dQAWA | coord (Days 1-11 — bundle-market coordinator; MAX_BUNDLE_SIZE=2 after Gate 3 falsified 3-source) — first submit | ~1100-1250 (n=6 4W/2L vs orbitfix @ μ=1174.2, Wilson 95% [0.30, 0.90]; n is below Rule 45 threshold but PI signed off) | 52894340 (_phase4_step1_FND, μ=1093.0) | ✅ PI explicit "Submit now" |
