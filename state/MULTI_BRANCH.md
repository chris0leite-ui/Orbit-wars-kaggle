...
| Stateful commit ledger for chooser (wait_N>0 fire-through) | `claude/audit-workflow-performance-btjeK` (c461b9f, edc4c30) | 2026-05-20 | Rule 37 exhausted: ledger_soft 2/16 (Wlo=0.035), ledger_hard 0/16 in n=8 h2h. Root cause: wait_N>0 reservation has unmodeled DEFENSIVE value that the ledger drains. What-if rollout overstated lift (recorded opp went passive after divergence); reactive h2h punishes the depletion. Audit: `audit/2026-05-21-ledger-validation.md`. Code stays env-gated `BASELINE_LEDGER=off` (default). |
| Opportunistic-opp + ledger combo (defensive-pricing re-attempt) | `claude/review-skills-improvements-moKOR` (7ddc7e6, 8c2d99c-pending) | 2026-05-23 | Variant #3 on ledger axis. Hypothesis: punish drained-source snipes in rollout → re-enable ledger. Falsified n=1 (seed=42, 2P self-play): VARIANT lost, 45% tiny launches vs PROD's 9% — opportunistic opp's aggressive baseline-sniping made IDLE look bad in BOTH branches, INFLATING small fire-now deltas (probe seed 596216766 turn 55: 2-ship fire-now delta 0 → +343; wait-bundle drain delta +167 → -640 as intended, but the dribble inflation dominated). Code retained env-gated `BASELINE_OPP_TIER=opportunistic` (default off); do NOT enable. |
...
|---|---|---|---:|---|---|
| 2026-05-23 22:30 | review-skills-improvements-moKOR | baseline (V3 wave + leaf-Δ gate + planet_positions cache) — wave variant 3 routes the staggered-attack joint action through the value head; cache fix delivers -45% per-turn p50 latency by sharing planet position arrays across same-turn predict_fleet_fate calls | ~1000-1080 (n=16 = 12/16 = 75% wins vs baseline_full Wilson-lo=0.505 clears Rule 45 lift-claim minimum 0.50; max-turn = 1199ms vs prior 2473ms; precedent: sibling-branch identical 12/16 result settled μ ~963-985 with separate perf approach) | 52959167 (orbitfix_kt_p23 v3, μ=973.0) | ✅ PI explicit "submit" 2026-05-23 22:25 |
