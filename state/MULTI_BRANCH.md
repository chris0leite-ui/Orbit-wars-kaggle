...
| State | Evidence | Next |
|---|---|---|
| **Rungs 1–4 BUILT + committed, default OFF** (`BASELINE_CHOOSER=greedy`, `agents/baseline/chooser_greedy.py`). Conditional sequential greedy + CELF + coalition atoms + shallow/deep horizon, on the deterministic `fast_sim` oracle (exact marginal gains). Champion (trajectory) byte-identical. **n=8 smoke (2026-06-02): first build is WEAKER than champion** — greedy 9/16 (56%) vs v7_0 vs champion's 16/16 (100%), same CRN seeds. NOT a bug: greedy is 3× FASTER per turn (p50 225 vs 728ms) → **under-committing**. Conditional greedy at shallow_h=12 prunes launches as ≤0 marginal gain because the `favor` leaf doesn't see captures MATURE within 12 ticks (the documented short-horizon undervaluation / hoarding myopia). Correctly kills waste, then under-expands. | Full framing + 5-rung sequence: `knowledge-base/concepts/joint-coordination-planner.md`. 6 oracle tests green (teamwork capture via atom, conditional decline of redundant launch w/ lock lifted, determinism, lazy==exact, anytime). Bench PASS (p95 246 / max 467ms, zero >1000ms). joint_sync + bundle still green. | **TUNE the under-expansion** (next session): (1) raise `BASELINE_GREEDY_SHALLOW_H` (e.g. 20–25) and/or turn on `BASELINE_GREEDY_CONFIRM=1` so captures register; (2) add un-gated capture credit at the leaf (mirror `EXPAND_CREDIT_WEIGHT`) so far captures aren't pruned; (3) sweep `BASELINE_GREEDY_MIN_DELTA`. Then proper n≥32 + multi-opp panel (Rules 43/45) vs champion before any promotion. Distinct from the closed sync-coalition operationalisation — this is a new joint *search*. |
