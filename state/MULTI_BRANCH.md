...
>
> **Status:** ✅ **SUBMITTED 2026-06-03 — sub `53336920` (`champ_refine_adaptivek.py`)**,
> PI explicit repeated "submit now". Bundle: refine + adaptive-K + table baked
> (`scripts/_build_refine_adaptivek_bundle.sh`). Rule 46c GREEN (322-step play
> smoke). Rule 2 timing GREEN (`fast.py bench` 880 turns: p95 633 / max 777ms,
> 0 over 1000ms). **Rule 42 outcome:** new rolling pair = `53336920` (refine) +
> `53332500` (computeByShips); **EVICTED `53324164` champ_adaptiveK_on μ~1170**
> from the rolling pair (candidate beats it 70% h2h ⇒ predicted μ ≥ 1170,
> eviction defensible; adaptiveK 1170 is now the backstop *behind* the rolling
> window — recoverable by resubmit if refine settles low). **Rule 43 panel was
> IN-FLIGHT at submit:** h2h vs champion CLEARED (40/57 = 70.2%, Wilson-lo
> 0.573 ≥ 0.55) but the three weak-opponent legs (v7_0 / v4_planner / v3.5.1)
> had not completed — **PI override** (panel-in-flight submit, precedent: the
> state_driven_k and joint_sync rows below). Local→live μ is noisy; this is a
> calibration probe on a strong local result.
>
...
|---|---|---|---:|---|---|
| 2026-06-03 ~17:55 | claude/kaggle-submission-strategy-JzIAr | **champ_refine_adaptivek** (sub **53336920**) — champion + adaptive-K + **AUGMENT teamwork refiner** (`BASELINE_CHOOSER=refine`: run champion verbatim, then add only oracle-positive 2-source sync coalitions that don't conflict with the champion's locks; never removes a champion launch). Reverses the 2026-06-02 "teamwork doesn't arise" closure (weak-opponent confound, Rule 41). | ~1170+ — **h2h vs adaptive-K champion 40/57 = 70.2%, Wilson-lo 0.573** (clears Rule 45 for the h2h leg); paired +13/−4/+9 net on 16 matched seeds. Rule 46c + bench (max 777ms, 0>1000) GREEN. **Rule 43 weak-opp legs (v7_0/v4_planner/v3.5.1) NOT complete at submit** (panel in-flight; killed by container restart). | 53324164 (**champ_adaptiveK_on, μ~1170.4** — older half of rolling pair). Backstop position-2: 53332500 (computeByShips, uncalibrated). Evicted 1170.4 ≈ predicted ⇒ **Rule 42 borderline; PI explicit override**. adaptiveK 1170 recoverable by resubmit if refine settles low. | ✅ PI explicit, repeated "submit now" (2026-06-03), informed: evicts adaptiveK μ~1170, refine uncalibrated live, Rule 43 panel weak-legs incomplete. |
