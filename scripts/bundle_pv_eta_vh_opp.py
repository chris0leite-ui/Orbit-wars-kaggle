"""Bundler for `agents/baseline_pv_eta_vh_opp` — Tier 2 opp-model wrapper.

Mirrors `scripts/bundle_pv_eta_vh.py`. The wrapper is env-var-only (no
Python wrapping code). Bundle structure:

    1. Wrapper env-var preamble (from agents/baseline_pv_eta_vh_opp/main.py).
    ...
    3. `_OPP_BOOSTER_B64` patched into the inlined `lib/opp_model.py`
       body so the shot-validator booster is available at submit time
       without lightgbm.

At `BASELINE_OPP_TIER=0`, this submission is byte-equivalent to bare
pv_eta. The Tier-2 policy itself falls back to Tier 1 if the booster
fails to load — never silent garbage launches.
"""
...
DEFAULT_WRAPPER = REPO / "agents" / "baseline_pv_eta_vh_opp"
DEFAULT_OUT = REPO / "submissions" / "baseline_pv_eta_vh_opp.py"
...
# Patch target inside the inlined opp_model body. Must match the
# literal in lib/opp_model.py exactly.
...
def _smoke_import(bundle_path: Path) -> None:
    spec = importlib.util.spec_from_file_location(
        "_bundle_pv_eta_vh_opp_smoke", bundle_path,
    )
...
def build(
    ...
    # `shot_features` + `_validator_tree_walker` must be in the bundle
    # for the Tier-2 opp policy to score candidates at submit time;
    # `value_head_features` carried for parity with the B.3 wrapper.
    ...
    header = (
        f"# Bundled by scripts/bundle_pv_eta_vh_opp.py — "
        f"{inner_dir.relative_to(REPO)} (inner) + "
        f"{wrapper_dir.relative_to(REPO)} (wrapper) + "
        f"{booster_path.relative_to(REPO)} (opp-booster B64).\n"
