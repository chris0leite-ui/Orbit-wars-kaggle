...
# producer_plus-local helper modules inlined between orbit_lite and main
# (imported in main as `from <name> import ...`; stripped at bundle time).
PRODUCER_PLUS_PRELUDE = ["shot_mlp"]
PRODUCER_PLUS_PRELUDE_EXPORTS = {"shot_mlp": ["apply_shot_mlp_veto"]}
PRODUCER_PLUS_DIR = REPO / "agents" / "producer_plus"
...
ENV_VARIANTS = {
    ...
    "vetorf4p_sync_shotmlp": {
        # Live stack (vetorf4p_sync, sub 53577315) + the learned shot-success
        # filter: a 24-feature MLP trained on live-episode outcomes drops
        # attack waves with P(hold target 10 turns post-arrival) below the
        # threshold. Reject-only, runs after the response veto. Threshold
        # baked here; sweep via --set PRODUCER_PLUS_SHOT_MLP=<t>.
        "PRODUCER_PLUS_MULTI_SIZE": "1",
        "PRODUCER_PLUS_OPP_PROJECTION": "1",
        "PRODUCER_PLUS_RESPONSE_VETO": "1",
        "PRODUCER_PLUS_REACTIVE_FLOOR": "0.5",
...
# Strip `from <prelude-module> import ...` (producer_plus-local helpers).
RE_PP_LOCAL_IMPORT = re.compile(
    r"^from (?:" + "|".join(PRODUCER_PLUS_PRELUDE) + r") import [^()\n]+$",
    re.MULTILINE,
)
...
def strip_imports(text: str, kind: str) -> str:
    if kind == "orbit_lite":
        ...
        text = RE_PP_LOCAL_IMPORT.sub("", text)
...
def build(env_vars: dict, out_path: Path) -> None:
    ...
    # PRODUCER_PLUS_SHOT_MLP is hardcoded into the gate function instead of
    # the env header: env vars are process-global, so a baked setdefault
    # leaks the threshold into a same-process opponent bundle (the
    # 2026-05-22 env-var-leak friction) and turns an A/B into a mirror.
    env_vars = dict(env_vars)
    shot_mlp_hardcode = env_vars.pop("PRODUCER_PLUS_SHOT_MLP", None)
    ...
    # Prelude modules are exec'd into a synthetic module object instead of
    # being flattened: their internal names (e.g. shot_mlp's float
    # fleet_speed) must NOT shadow orbit_lite's tensor functions in the
    # bundle's flat namespace. Only the listed exports are re-bound.
    for mod_name in PRODUCER_PLUS_PRELUDE:
        src = (PRODUCER_PLUS_DIR / f"{mod_name}.py").read_text()
        exports = PRODUCER_PLUS_PRELUDE_EXPORTS[mod_name]
        bind = "\n".join(
            f"{name} = _pp_{mod_name}_mod.{name}" for name in exports)
        parts.append(
            f"\n# === producer_plus.{mod_name} (namespaced) ===\n"
            "import types as _pp_types\n"
            f"_pp_{mod_name}_mod = _pp_types.ModuleType({mod_name!r})\n"
            f"exec(compile({src!r}, {mod_name + '.py'!r}, 'exec'), "
            f"_pp_{mod_name}_mod.__dict__)\n"
            f"{bind}\n"
        )

    main_src = PRODUCER_PLUS_MAIN.read_text()
    if shot_mlp_hardcode is not None:
        needle = '    raw = os.environ.get("PRODUCER_PLUS_SHOT_MLP")\n'
        if needle not in main_src:
            raise SystemExit("shot-mlp gate anchor not found in main.py")
        main_src = main_src.replace(
            needle,
            f"    raw = {shot_mlp_hardcode!r}  "
            "# hardcoded at bundle time (env-leak-proof A/B)\n",
            1,
        )
