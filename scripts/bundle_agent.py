...
#   - `from agents.<name>.X import` (modular agent pattern, ported from
#     btjeK commit 4094aa1 — bundle ran locally because agents.<name> was
...
def bundle(
    ...
    # Inline agent submodules (e.g. agents/baseline/{value,proposer,chooser,
    # migration_solver}.py) before main.py. Modular agent pattern — main.py
    # imports symbols from sibling files; without inlining the bundle would
    # NameError after the intra-package import lines are stripped.
    # Topological order by `from agents.<name>.X import` references. Ported
    # from btjeK commit 4094aa1 (2026-05-17).
    if agent_dir.is_dir():
...
def _topo_sort_agent_submodules(agent_dir: Path) -> list[tuple[str, str]]:
    ...
    inlining). Cycles → ValueError (in practice indicate a layering bug).

    Ported from btjeK commit 4094aa1.
    """
    pkg = agent_dir.name
    ...
    for path in sorted(agent_dir.glob("*.py")):
        ...
        for line in src.splitlines():
            m = dep_re.match(line)
            if m and m.group(1) in submodules and m.group(1) != mod_name:
                deps[mod_name].add(m.group(1))
    ...
    while pending:
        ...
        if not ready:
            raise ValueError(f"cycle in agent submodule deps: {pending}")
        for n in ready:
