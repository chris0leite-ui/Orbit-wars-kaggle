"""Bundler for `agents/baseline_pv_eta_vh_dist_composite` — Phase 6c composite.

Composes the distilled-ladder opp model (Tier 2 v2) AND the B.3 CRN-advantage
value head into a single submission. Patches BOTH blobs into the inlined
bundle:
...
Two-blob bundles are why this isn't just a one-line clone of either base
bundler — both patch sites must hit, and both source artifacts must exist
before invoking.
"""
...
DEFAULT_WRAPPER = REPO / "agents" / "baseline_pv_eta_vh_dist_composite"
DEFAULT_OUT = REPO / "submissions" / "baseline_pv_eta_vh_dist_composite.py"
...
def _smoke_import(bundle_path: Path) -> None:
    spec = importlib.util.spec_from_file_location(
        "_bundle_pv_eta_vh_dist_composite_smoke", bundle_path,
    )
...
def build(
    ...
    # shot_features is intentionally NOT required: it's only referenced
    # in the dead-code legacy slow path of `trained_logreg_policy`
    # (gated by `_DIST_USE_LITE_ENCODER`, default True). On 00JzI
    # `lib/shot_features.py` does not exist; adding it would require
    # cherry-picking the 23 KB file plus its scoring/world-model
    # dependency chain. The lite encoder path (production) needs only
    # `_validator_tree_walker`, `value_head_features`, `opp_features_lite`.
    for required in ("_validator_tree_walker",
    ...
    header = (
        f"# Bundled by scripts/bundle_pv_eta_vh_dist_composite.py — composite of:\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"  composite: distilled opp (Tier 2 v2) + B.3 value head")
    return 0
