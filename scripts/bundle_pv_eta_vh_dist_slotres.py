"""Bundler for `agents/baseline_pv_eta_vh_dist_slotres`.

Wrapper that sets `BASELINE_SLOT_RESERVATION=3/2/2` and
`BASELINE_WALLCLOCK_MS=800` on top of the composite (pv_eta + distilled
opp Tier 2 v2 + B.3 head λ=1.0). The slot-reservation logic itself
lives in `agents/baseline/chooser_trajectory.py:_parse_slot_reservation`
(default OFF when env var unset — preserves bundle-parity).
...
DEFAULT_WRAPPER = REPO / "agents" / "baseline_pv_eta_vh_dist_slotres"
DEFAULT_OUT = REPO / "submissions" / "baseline_pv_eta_vh_dist_slotres.py"
...
def _smoke_import(bundle_path: Path) -> None:
    spec = importlib.util.spec_from_file_location(
        "_bundle_pv_eta_vh_dist_slotres_smoke", bundle_path,
    )
...
def build(
    ...
    header = (
        f"# Bundled by scripts/bundle_pv_eta_vh_dist_slotres.py — composite + slot reservation:\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"  composite + slot reservation 3/2/2 + wallclock 800ms")
    return 0
